...
export default class Recipe {
    ...
    async getRecipe() {
        try {
            ...
            this.ingredients = ["1 medium head cauliflower, cut into florets",
                "1 egg",
    ...
    calcTime() {
        //Assuming that we need 15 min for each 3 ingrediendts
    ...
    parseIngredients() {
        ...
        const newIngredients = this.ingredients.map(el => {
            ...
            unitsLong.forEach((unit, i) => {
                ingredient = ingredient.replace(unit, unitsShort[i]);      //replace all longer terms with the same meaning into the same one
            });
